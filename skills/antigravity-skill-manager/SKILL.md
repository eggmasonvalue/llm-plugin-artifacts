...
1. **Locate**: Find the skill in `<repository-root>\skills\<skill-name>`.
2. **Edit**: Use `view_file` to read `SKILL.md` or scripts, then `replace_file_content` to make changes.
3. **Verify Symlinks**: Ensure the skill is correctly linked in `C:\Users\<username>\.gemini\antigravity\skills`.
